...
public class E09InsertionSort {
    ...
    public static <E> void insertionSort(E[] a, Comparator<E> c) {
        for(int i = 1; i <  a.length; i++) {
            ...
            E x = a[i];
            int j;
            for(j = i; j > 0 && c.compare(a[j-1], x) > 0; j--) {
                a[j] = a[j-1];
            }
            a[j] = x;
        }
    ...
    public static void main(String[] args) {
        ...
        insertionSort(sortedStrings);
        System.out.printf("insertionSort: %s\n", Arrays.toString(sortedStrings));
        ...
        insertionSort(sortedPersons, personComparator);
